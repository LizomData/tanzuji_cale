...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col, ImGuiButtonFlags flags)
{
    ...
    const ImVec2 padding = g.Style.FramePadding;
     ImRect bb(window->DC.CursorPos, window->DC.CursorPos + image_size + padding * 2.0f);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
    ...
    if (held && hovered)
    {
        bb = ImRect(bb.Min + image_size * 0.05, bb.Max - image_size * 0.05);
    }

    RenderFrame(bb.Min +ImVec2(THICKNESS * 0.5f, THICKNESS * 0.5f)*2.0f, bb.Max- ImVec2(THICKNESS * 0.5f, THICKNESS * 0.5f)* 2.0f, ImColor(0, 0, 0, 100), true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
    ...
    //±ß¿ò
    if (hovered)
        window->DrawList->AddRect(bb.Min + ImVec2(THICKNESS * 0.5f, THICKNESS * 0.5f), bb.Max - ImVec2(THICKNESS * 0.5f, THICKNESS * 0.5f), ImColor(255, 255, 255, 150), 0.0f, 0, 2.0);
    
    //±³¾°
    const ImU32 col = ((held && hovered) ? ImColor{ 255,255,255,100 } : hovered ? ImColor{ 255,255,255,50 } : ImColor{ 255,255,255,0 });
...
bool ImGui::Checkbox(const char* label, bool* v, ImU32 TextColor)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (label_size.x > 0.0f)
        RenderText(label_pos, label, 0,true,TextColor);
...
{
    return InputTextEx(label, "Ask me anything...", buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
}
bool ImGui::InputTextMultiline(int sty,const char* label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    return InputTextEx(sty,label, "Ask me anything...", buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        PushStyleColor(ImGuiCol_ChildBg, ImVec4(1.0f, 1.0f, 1.0f,0.5f));
        PushStyleColor(ImGuiCol_Text, ImVec4(0, 0, 0, 255));
        PushStyleColor(ImGuiCol_TextDisabled, ImVec4(0, 0, 0, 255));
        PushStyleVar(ImGuiStyleVar_ChildRounding, 1.0f);
        PushStyleVar(ImGuiStyleVar_ChildBorderSize, style.FrameBorderSize);
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0)); // Ensure no clip rect so mouse hover can reach FramePadding edges
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar);
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, 5.0f);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            ImU32 col = is_displaying_hint ? ImColor(0.50f, 0.50f, 0.50f, 1.00f) : ImColor(1, 1, 1, 255);          
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), ImColor(45.0f/255.0f, 139.0f / 255.0f, 96.0f / 255.0f,1.0f));
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, col, buf_display, buf_display_end, 0, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool ImGui::InputTextEx(int sty,const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (is_multiline) // Open group before calling GetID() because groups tracks id created within their scope (including the scrollbar)
        BeginGroup();
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(buf, 0, true, 200.0f) ;
    
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line

    const ImVec2 total_size = ImVec2( label_size.x , frame_size.y);
    ...
    if (is_multiline)
    {
        ...
        // We reproduce the contents of BeginChildFrame() in order to provide 'label' so our window internal data are easier to read/debug.
        PushStyleColor(ImGuiCol_ChildBg, ImVec4(1.0f, 1.0f, 1.0f,0.0f));
        PushStyleColor(ImGuiCol_Text, ImVec4(0, 0, 0, 255));
        PushStyleColor(ImGuiCol_TextDisabled, ImVec4(0, 0, 0, 255));
        PushStyleVar(ImGuiStyleVar_ChildRounding, 1.0f);
        PushStyleVar(ImGuiStyleVar_ChildBorderSize, style.FrameBorderSize);
        ...
        bool child_visible = BeginChildEx(label, id, frame_size, true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar);
        g.NavActivateId = backup_activate_id;
        PopStyleVar(3);
        PopStyleColor(3);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImVec2 marker_min = ImVec2(pos.x + 200.0f, pos.y);
            ImVec2 marker_max = ImVec2(pos.x + 200.0f + 10, pos.y + ImGui::GetTextLineHeight());

            ImGui::PushTextWrapPos(ImGui::GetCursorPos().x + 200.0f);
            PushStyleColor(ImGuiCol_Text, ImVec4(1, 1, 1, 255));
            PushStyleColor(ImGuiCol_TextDisabled, ImVec4(1, 1, 1, 255));

            ImGui::Text(buf_display);
            //draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos , col, buf_display, buf_display_end, 200.0f, is_multiline ? NULL : &clip_rect);
            draw_window->DrawList->AddRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), IM_COL32(255, 255, 0, 255));
            
            draw_window->DrawList->AddRectFilled(marker_min, marker_max, IM_COL32(255, 0, 255, 255));
